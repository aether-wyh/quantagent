...
def _run_strategy_agent_with_fallback(state: WorkflowState, strategy_agent: StrategyAgent) -> WorkflowState:
    try:
        return strategy_agent.run(state)
    except Exception as exc:
        message = str(exc)
        known_backend_errors = (
            "Failed to connect to DolphinDB",
            "resolved universe is empty",
            "Unknown dataset",
        )
        if not any(item in message for item in known_backend_errors):
            raise
        ...
        try:
            print("[2/3] Falling back to synthetic required_data because the data backend is unavailable")
            return strategy_agent.run(state)
        finally:
            strategy_agent_module.load_required_data = original_loader


def _build_summary(experiment_file: Path, state: WorkflowState) -> dict[str, object]:
    ...
    return {
        ...
        "validate_feedback": state.get("validate_feedback", ""),
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run HypothesisAgent -> StrategyAgent -> ValidateAgent with real LLM calls.",
    )
    parser.add_argument(
        "--experiment-file",
        type=Path,
    ...
    parser.add_argument(
        "--require-validate-pass",
        action="store_true",
        help="Exit with code 1 when ValidateAgent does not pass.",
    )
    ...
    print(f"[1/3] Running HypothesisAgent on {experiment_file}")
    state = hypothesis_agent.run(state)
    print("[1/3] hypothesis:")
    print(state.get("hypothesis", ""))
    print()

    print("[2/3] Running StrategyAgent")
    state = _run_strategy_agent_with_fallback(state, strategy_agent)

    strategy_result = state.get("strategy_result", {})
    strategy_output_keys = list(strategy_result.get("strategy_output", {}).keys()) if isinstance(strategy_result, dict) and isinstance(strategy_result.get("strategy_output"), dict) else []
    print(f"[2/3] strategy_output keys: {strategy_output_keys}")

    print("[3/3] Running ValidateAgent")
    state = validate_agent.run(state)

    summary = _build_summary(experiment_file, state)
    print("[3/3] validate summary:")
    ...
    passed = bool(cast(dict[str, object], summary.get("optimization_result", {})).get("passed"))
    if args.require_validate_pass and not passed:
        print("ValidateAgent did not pass; exiting with status 1 because --require-validate-pass was set.")
        return 1
