"""V3 streamed saved-data accounting; original matching/fees/ledger remain frozen.
...
VERSION = "meta-v3-saved-execution-stream-v1"
...
SOURCE_NAMES = ("raw_saved_research.py", "raw_portfolio_backtest.py", "raw_share_ledger.py",
                "corporate_action_adapter.py", "saved_execution.py")
...
def simulate_streamed_portfolio(*, checkpoint, **kwargs):
    """Call the frozen accounting body with a new write-ahead observer."""
...
def engine_sources(*, archive_storage_policy=None):
    from .archive_codec import validate_policy
    validate_policy(archive_storage_policy)
    sources = {name: hashlib.sha256((Path(__file__) if name == "saved_execution.py" else FROZEN / name).read_bytes()).hexdigest() for name in SOURCE_NAMES}
    if archive_storage_policy is not None:
        sources['archive_codec.py'] = hashlib.sha256(Path(__file__).with_name('archive_codec.py').read_bytes()).hexdigest()
    return sources
...
def freeze_saved_research_plan(*, identity, codes, calendar, targets, source_artifacts,
                               ...
                               fixture_only=False, archive_storage_policy=None):
    ...
    plan = {"version": VERSION, "identity": identity, "codes": codes, "calendar": calendar,
            ...
            "fixture_only": fixture_only, "engine_sources": engine_sources(archive_storage_policy=archive_storage_policy), "resource_budget": RESOURCE_BUDGET,
    ...
    if archive_storage_policy is not None:
        plan['archive_storage_policy'] = archive_storage_policy
    plan = _copy(plan)
...
def _validate_plan(plan):
    from .archive_codec import validate_policy
    storage = plan.get('archive_storage_policy')
    _require('archive_storage_policy' not in plan or storage is not None, 'explicit archive policy cannot be null')
    validate_policy(storage)
    ...
    expected_sources = set(SOURCE_NAMES) | ({'archive_codec.py'} if storage is not None else set())
    _require(set(plan.get("engine_sources", {})) == expected_sources and all(_sha(x) for x in plan["engine_sources"].values()), "engine identities invalid")
...
class SavedRawResearch:
    ...
    def create(cls, root, plan):
        ...
        with sqlite3.connect(root / "research.sqlite3") as db:
            ...
            if storage is None:
                ...
                db.execute('''CREATE TABLE records_zlib (
                    seq INTEGER PRIMARY KEY, previous_sha256 TEXT NOT NULL, payload BLOB NOT NULL,
                    stored_bytes INTEGER NOT NULL, stored_sha256 TEXT NOT NULL,
                    decoded_bytes INTEGER NOT NULL, decoded_sha256 TEXT NOT NULL, sha256 TEXT NOT NULL)''')
                db.execute('INSERT INTO metadata VALUES (?,?)', ('archive_storage_policy', _json(storage)))
            db.executemany("INSERT INTO metadata VALUES (?, ?)", [("plan", _json(plan)), ("plan_sha256", expected),
                ("usage", _json({"bytes":0,"event_intents":0,"records":0,
                                  **({'stored_bytes':0} if storage is not None else {})}))])
    ...
    def _storage(db):
        from .archive_codec import validate_policy
        row = db.execute("SELECT value FROM metadata WHERE key='archive_storage_policy'").fetchone()
        storage = json.loads(row[0]) if row else None
        _require(row is None or storage is not None, 'archive storage metadata cannot be null')
        validate_policy(storage)
        tables = {r[0] for r in db.execute("SELECT name FROM sqlite_master WHERE type='table'")}
        _require(('records_zlib' in tables and 'records' not in tables) if storage is not None else
                 ('records' in tables and 'records_zlib' not in tables), 'archive policy and record layout differ')
        return storage
    ...
    def _read(self, expected_plan_sha256):
        ...
        with sqlite3.connect(uri, uri=True) as db:
            ...
            _require(storage == plan.get('archive_storage_policy'), 'archive policy differs from frozen plan')
            ...
            records, previous, total, stored_total = [], "0" * 64, 0, 0
            def record_rows():
                if storage is None:
                    yield from db.execute("SELECT seq,previous_sha256,body,sha256 FROM records ORDER BY seq")
                    return
                from .archive_codec import decode_record, stored_limit, VERSION as codec
                # Bound the BLOB using its SQL length before fetching/allocating
                # it; use this same read transaction for its original metadata.
                cursor = db.execute('SELECT seq,previous_sha256,length(payload),stored_bytes,stored_sha256,decoded_bytes,decoded_sha256,sha256 FROM records_zlib ORDER BY seq')
                for seq, prior, size, count, stored_hash, decoded_size, decoded_hash, chain_hash in cursor:
                    _require(type(decoded_size) is int and 0 <= decoded_size <= min(MAX_RECORD, MAX_ARCHIVE-total)
                             and type(size) is int and type(count) is int and size == count and 0 <= size <= stored_limit(decoded_size)
                             and stored_total+size <= MAX_ARCHIVE and len(records) < MAX_RECORDS, 'saved encoded evidence bound exceeded')
                    blob = db.execute('SELECT payload FROM records_zlib WHERE seq=?', (seq,)).fetchone()[0]
                    decoded = decode_record({'codec':codec, 'payload':blob, 'stored_bytes':size, 'stored_sha256':stored_hash,
                                             'decoded_bytes':decoded_size, 'decoded_sha256':decoded_hash}, storage, decoded_limit=MAX_RECORD)
                    yield seq, prior, decoded.decode('utf-8'), chain_hash
            for seq, prior, body, digest in record_rows():
                body_bytes = len(body.encode('utf-8'))
                total += body_bytes
                if storage is not None:
                    stored_total += db.execute('SELECT stored_bytes FROM records_zlib WHERE seq=?', (seq,)).fetchone()[0]
                _require(len(records) < MAX_RECORDS and total <= MAX_ARCHIVE and body_bytes <= MAX_RECORD, "saved evidence bound exceeded")
            ...
            expected_usage = {"bytes":total,"records":len(records),
                ...
                **({'stored_bytes':stored_total} if storage is not None else {})}
            _require(usage == expected_usage, "resource counters differ from committed records")
            self._verified_storage_usage = dict(expected_usage)
    ...
    def _append(self, kind, payload):
        ...
        with sqlite3.connect(self.db) as db:
            ...
            storage = self._storage(db)
            table = 'records_zlib' if storage is not None else 'records'
            prior = db.execute('SELECT seq,sha256 FROM '+table+' ORDER BY seq DESC LIMIT 1').fetchone()
            ...
            if storage is None:
                ...
                from .archive_codec import encode_record, stored_limit
                encoded = encode_record(body.encode('utf-8'), storage, decoded_limit=MAX_RECORD)
                encoded_reserve = (reserve // MAX_RECORD) * stored_limit(MAX_RECORD)
                _require(usage['stored_bytes'] + encoded['stored_bytes'] + encoded_reserve <= MAX_ARCHIVE,
                         'encoded archive lacks reserved persistence bytes')
                db.execute('INSERT INTO records_zlib VALUES (?,?,?,?,?,?,?,?)',
                           (seq, previous, encoded['payload'], encoded['stored_bytes'], encoded['stored_sha256'],
                            encoded['decoded_bytes'], encoded['decoded_sha256'], digest))
                usage['stored_bytes'] += encoded['stored_bytes']
    ...
    def execute_saved(self, *, expected_plan_sha256):
        ...
        with _lease(self.root):
            ...
            if records:
                _require(plan["engine_sources"] == engine_sources(archive_storage_policy=plan.get('archive_storage_policy')), "engine drift; use saved-only inspection")
            ...
            self._append("execution_intent", {"plan_sha256": expected_plan_sha256, "engine_sources": engine_sources(archive_storage_policy=plan.get('archive_storage_policy')), **FLAGS})
            try:
                _require(plan["engine_sources"] == engine_sources(archive_storage_policy=plan.get('archive_storage_policy')), "engine drift since plan freeze")
                ...
                result = simulate_streamed_portfolio(daily_data=frame, calendar=plan["calendar"],
                    ...
                    start_date=plan["calendar"][0], end_date=plan["calendar"][-1], checkpoint=checkpoint)
    ...
    def reconcile_saved_only(self, *, expected_plan_sha256):
        ...
        resources = {"persisted_payload_bytes":sum(len(_json(r).encode()) for r in records),
        ...
        if plan.get('archive_storage_policy') is not None:
            from .archive_codec import stored_limit
            resources.update(persisted_payload_bytes=self._verified_storage_usage['stored_bytes'],
                             stored_payload_bytes=self._verified_storage_usage['stored_bytes'],
                             decoded_payload_bytes=self._verified_storage_usage['bytes'],
                             decoded_pending_write_reserved_bytes=resources['pending_write_reserved_bytes'],
                             decoded_terminal_reserved_bytes=resources['terminal_reserved_bytes'],
                             pending_write_reserved_bytes=stored_limit(MAX_RECORD) if pending else 0,
                             terminal_reserved_bytes=0 if terminal else stored_limit(MAX_RECORD),
                             storage_codec=plan['archive_storage_policy']['version'])
