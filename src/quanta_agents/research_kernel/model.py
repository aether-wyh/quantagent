...
from .store import exclusive_lock, serial, write_json
        ...
        if kernel.status()["model_calls"] >= kernel.config["budget"]["max_model_calls"]:
            with kernel.store.transaction() as db:
                ...
                if not kernel.store.meta("model_stopped", False):
                    ...
                    kernel.store.event(db, "model_budget_exhausted", {"reason": "No additional research model call is authorized by this frozen study budget",
