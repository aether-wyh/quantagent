...
from .controller import ResearchKernel
from .store import serial, write_json
...
def run_supervised(kernel):
    ...
    result = run_job([sys.executable, "-m", "quanta_agents.research_kernel.cli", "run-worker",
...
def main(argv=None):
    ...
    parser.add_argument("command", choices=["init", "register", "import-v6", "import-catalog", "action",
        "status", "context", "evidence", "run", "run-worker", "iterate", "recover-call", "retry", "stop"])
    ...
    if args.command == "init":
        ...
        pending = kernel.status()["runs"]
        if pending.get("queued", 0) or pending.get("running", 0):
        ...
        for _ in range(args.steps):
            ...
            if decision.get("status") in {"stopped", "model_budget_exhausted"}:
                break
            if kernel.status()["runs"].get("queued", 0):
