...
## 33. A17：为中证500 指数增强寻找更多超额（2026-09-17 起，进行中）

记录见 docs/research/framework_review_20260913/A17_MORE_ALPHA_LOG.zh-CN.md。用户批准六个目标并要求逐个达到预期增量：① 降噪（5 种子平均 + 逐年滚动选规则，夏普 +0.1~+0.2）；② 目标对齐（并集 / 500 样本加权、头部加权、多头与空头分开建模，+1~+2pt）；③ 结构性收益（调样预测、除息、涨跌停与复牌，每项 +0.3~+1pt）；④ 执行（开盘成交价、不交易带、按账户规模缩 500 桶名字数，+0.5~+1.5pt）；⑤ 环境切换（离散度与多头端盈亏驱动的超配开关）；⑥ 定向化学反应（500 内多头为目标的交叉复合，+0.5~+1pt）。总目标：研究期 +8%~+10%、夏普 ≥ 1.5。反思结论：瓶颈不是因子数量，而是训练目标与任务错位、缺少任务自带的结构性收益、评价噪声与执行损耗。

**A17 进度（2026-09-17 凌晨—清晨）**：
- ① 降噪：五种子平均分数 = 单种子均值，**无水平增量**（价值只是不再抽签）；逐年滚动选规则每年选到同一条，诚实口径 2021—2024 +5.3% / 2.18。顺带发现换手中性标签在 c3-A 规则上五种子口径 +0.3pt / 夏普 +0.13（达标）。
- ③ 结构性收益：调样预测（Opus 子代理）调出精确率 0.84 / 0.86，追加式否决 +0.3pt / +0.08（单项未过门槛，叠在 c4 上 +0.5pt / +0.14）；涨跌停与复牌无剩余收益；除息不属于量价、不做。**新增两项达标**：用价格反推指数真实权重（纯 500 桶跟踪误差减半以上，研究期夏普 +0.1 ~ +0.2，前向 +1.2 ~ +1.4pt）；另一只桶只从中证1000 里选（+0.2pt / 夏普 +0.12 ~ +0.16）。
- ④ 执行：开盘集合竞价已是最优成交时点，延后每年 −0.3 ~ −1.0pt；发现 200 万账户把高价股整体丢掉是 2025—2026 整手损失的全部来源（2026 年这批股票占 500 桶 20%、年化跑赢 26%），分价格段的整手取整把 2026 冻结期整手代价从约 5pt 降到 0。
- ⑤ 环境切换：证伪。⑥ 定向交叉复合：筛选阶段证伪（训练年的化学反应在 2019—2024 回到父代水平）。② 目标对齐：证伪（并集 / 500 内样本加权在种子噪声内，头部加权 −1pt，多头 / 空头拆成二分类模型 −2 ~ −3pt；全 A 排序回归仍是最好的训练目标）。
- **候选 c4 已冻结**（换手中性标签五种子分数 + c3-A 规则 + 反推权重 + 另一只桶限中证1000；实盘配套分价格段整手取整）：研究期 +6.0% / 2.19（回撤 −3.1%），2025 +0.5%，2026 冻结期 +7.8% / 1.46（前向回撤 −6.1%）；200 万整手口径研究期 +5.3% / 1.87、2025 −0.4%、2026 +6.2% / 1.06（现行 c2 同口径 2025 −3.9%、2026 +1.3% / 0.15）。实盘模块的三个新开关默认关闭（c2 输出逐股回归一致），日更链新增 c4 与"c3-A + 新取整"两条并行目标（失败不影响主链）。
- 数据侧留档：研究面板的成分切换日比真实生效日晚 10—12 个交易日；12 月调样生效日（2026-12-11 收盘后）当天须核对实盘成分已切换。
